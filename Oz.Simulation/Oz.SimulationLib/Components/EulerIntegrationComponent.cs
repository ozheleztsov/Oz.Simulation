...
public class EulerIntegrationComponent : SimComponent
{
    ...
    public EulerIntegrationComponent(ISimContext context, ISimObject owner, ILogger logger, Guid? id = null, string? name = null) : base(context, owner, logger, id, name)
    {
    }
    ...
    private (Vector3 Velocity, Vector3 Position) GetUpdatedPositionVelocities(List<IntegrationObject> previousObjects, int targetObjectIndex, double deltaTime)
    {
        ...
        var position = targetObject.Transform.Position + (deltaTime * GetVelocity(previousObjects, targetObjectIndex));
    ...
    private Vector3 GetVelocity(IReadOnlyList<IntegrationObject> previousObjects, int targetObjectIndex) =>
        previousObjects[targetObjectIndex].Velocity.Velocity;
}