...
public class OzBlogDataService : IBlogDataService
{
    ...
    public OzBlogDataService(IOptions<OzDataOptions> options)
    {
    ...
    public async Task<IEnumerable<BlogEntryResponseData>> GetBlogEntriesAsync(Func<BlogEntry, bool> predicate, CancellationToken cancellationToken = default)
    {
        await CreateBlogTableIfNotExists(cancellationToken).ConfigureAwait(false);
        List<BlogEntry> blogEntries = new();
        await foreach (var blogEntry in _tableClient.QueryAsync<BlogEntry>(x => predicate(x), cancellationToken: cancellationToken))
        {
