...
public class HitTestEdge
{
    ...
    {
        var projPoint1 = objectToViewportTransform.Transform(_p1);
        var projPoint2 = objectToViewportTransform.Transform(_p2);
...
public class InteractiveVisual3D : ModelVisual3D
{
    ...
    /// </summary>
    private static readonly DependencyProperty VisualProperty =
    ...
    /// </summary>
    internal UIElement? InternalVisual { get; private set; }

    /// <summary>
    ///     The visual brush that the internal visual is contained on.
    /// </summary>
    private VisualBrush InternalVisualBrush { get; set; }
    ...
    {
        // our final edge list
        var hitTestEdgeList = new List<HitTestEdge>();
        var adjInformation = new Dictionary<Edge, EdgeInfo>();

        // store some important info in local variables for easier access
        if (_content == null)
        {
            throw new InvalidOperationException(nameof(_content));
        }
        
        var contentGeom = (MeshGeometry3D)_content.Geometry;
        var positions = contentGeom.Positions;
        var textureCoords = contentGeom.TextureCoordinates;
        var triIndices = contentGeom.TriangleIndices;
        ...
        // the local variable is named cameraToObjectTransform.
        var cameraToObjectTransform = MathUtils.TryTransformToCameraSpace(this, out _, out var success);
        if (!success)
        {
        ...
        var objectToViewportTransform = MathUtils.TryTransformTo2DAncestor(this, out _, out success);
        if (!success)
        {
            return new List<HitTestEdge>();
        }
        ...
        var sameAsBefore = _lastVisCorners != null;
        ...
        var camPosObjSpace = cameraToObjectTransform.Transform(new Point3D(0, 0, 0));
        ...
        foreach (var edge in adjInformation.Keys)
        {
            var ei = adjInformation[edge];
        ...
        for (var i = 0; i < hitTestEdgeList.Count; i++)
        {
        ...
        Point3D camPosObjSpace)
    {
        ...
        var normal = Vector3D.CrossProduct(p[1] - p[0], p[2] - p[0]);
        var dirToCamera = camPosObjSpace - p[0];

        // ignore any triangles that have a normal of (0,0,0)
        if (!(normal.X == 0 && normal.Y == 0 && normal.Z == 0))
        {
            var dotProd = Vector3D.DotProduct(normal, dirToCamera);
        ...
        List<HitTestEdge> edgeList)
    {
        ...
        var uvList = new List<Point>();

        // loop over the visual's texture coordinate bounds
        for (var i = 0; i < tc.Length; i++)
        {
            var visEdgeStart = tc[i];
            var visEdgeEnd = tc[(i + 1) % tc.Length];
            ...
            var skipListProcessing = false;
            for (var j = 0; j < uv.Length; j++)
            {
                var uv1 = uv[j];
                var uv2 = uv[(j + 1) % uv.Length];
                var p3D1 = p[j];
                var p3D2 = p[(j + 1) % p.Length];
                      ...
                      Math.Min(visEdgeStart.Y, visEdgeEnd.Y) > Math.Max(uv1.Y, uv2.Y)))
                {
                    ...
                    var areCoincident = false;
                    var dir = uv2 - uv1;
                    var t = IntersectRayLine(uv1, dir, visEdgeStart, visEdgeEnd, out areCoincident);
                    ...
                    if (t >= 0 && t <= 1)
                    {
                        var intersUV = uv1 + (dir * t);
                        var intersPoint3D = p3D1 + ((p3D2 - p3D1) * t);

                        var visEdgeDiff = (visEdgeStart - visEdgeEnd).Length;
    ...
    private bool IsPointInTriangle(Point p, IReadOnlyList<Point> triUvVertices, IReadOnlyList<Point3D> tri3DVertices, out Point3D inters3DPoint)
    {
        var denom = 0.0;
        inters3DPoint = new Point3D();

        var a = triUvVertices[0].X - triUvVertices[2].X;
        var b = triUvVertices[1].X - triUvVertices[2].X;
        var c = triUvVertices[2].X - p.X;
        var d = triUvVertices[0].Y - triUvVertices[2].Y;
        var e = triUvVertices[1].Y - triUvVertices[2].Y;
        var f = triUvVertices[2].Y - p.Y;
        ...
        var lambda2 = ((a * f) - (c * d)) / denom;

        if (lambda1 < 0 || lambda1 > 1 || lambda2 < 0 || lambda2 > 1 || lambda1 + lambda2 > 1)
        {
            return false;
        }
    ...
    /// <summary>
    ///     Intersects a ray with the line specified by the passed in end points.  The parameterized coordinate along the ray
    ///     of
    ...
    private double IntersectRayLine(Point o, Vector d, Point p1, Point p2, out bool coinc)
    {
        ...
        if (dx == 0)
        {
            ...
            coinc = Math.Abs(o.X - p1.X) < 1e-5f;
            return -1;

        }

        // now need to do more general intersection
        var numer = ((o.X - p1.X) * dy / dx) - o.Y + p1.Y;
        var denom = d.Y - (d.X * dy / dx);

        // if denominator is zero, then the lines are parallel
        if (denom == 0)
        {
            var b0 = (-o.X * dy / dx) + o.Y;
            var b1 = (-p1.X * dy / dx) + p1.Y;

            coinc = Math.Abs(b0 - b1) < 1e-5f;
            return -1;
        }
    ...
    private void ProcessTriangleEdges(IReadOnlyList<Point3D> p,
        IReadOnlyList<Point> uv,
        Point[] tc,
        PolygonSide polygonSide,
        List<HitTestEdge> edgeList,
        IDictionary<Edge, EdgeInfo> adjInformation)
    {
        ...
        for (var i = 0; i < p.Count; i++)
        {
            ...
            var alreadyAdded = edgeInfo.hasBackFace && edgeInfo.hasFrontFace;
        ...
        List<HitTestEdge> edgeList)
    {
        ...
        {
            var visEdgeStart = bounds[i];
            var visEdgeEnd = bounds[(i + 1) % bounds.Length];
    ...
    private bool IsPointInPolygon(Point[] polygon, Point p)
    {
        ...
        {
            var crossProduct = Vector.CrossProduct(polygon[(i + 1) % polygon.Length] - polygon[i],
                polygon[i] - p);

            var currSign = crossProduct > 0;
    ...
    {
        var foundMaterialToSwap = false;
        var materialStack = new Stack<Material>();
        materialStack.Push(material);

        while (materialStack.Count > 0)
        {
            var currMaterial = materialStack.Pop();

            if (currMaterial is DiffuseMaterial)
            {
                var diffMaterial = (DiffuseMaterial)currMaterial;
                if ((bool)diffMaterial.GetValue(IsInteractiveMaterialProperty))
                {
            ...
            {
                var emmMaterial = (EmissiveMaterial)currMaterial;
                if ((bool)emmMaterial.GetValue(IsInteractiveMaterialProperty))
                {
            ...
            {
                var specMaterial = (SpecularMaterial)currMaterial;
                if ((bool)specMaterial.GetValue(IsInteractiveMaterialProperty))
                {
            ...
            {
                var matGroup = (MaterialGroup)currMaterial;
                foreach (var m in matGroup.Children)
                {
            ...
            else
            {
                throw new ArgumentException(
                    "material needs to be either a DiffuseMaterial, EmissiveMaterial, SpecularMaterial or a MaterialGroup",
                    "material");
            }
        }
        ...
        {
            throw new ArgumentException(
                "material needs to contain at least one material that has the IsInteractiveMaterial attached property",
                "material");
        }
    }
    ...
    {
        var imv3D = (InteractiveVisual3D)sender;
        ...
        if (imv3D.InternalVisual != null)
        {
            ...
            {
                var oldVisualDecorator = (VisualDecorator)ad.Child;
        ...
        {
            var visDecorator = new VisualDecorator();
    ...
    public static bool GetIsInteractiveMaterial(UIElement element) =>
        (bool)element.GetValue(IsInteractiveMaterialProperty);
    ...
    private struct Edge
    {
        ...
        public readonly Point3D _start;
        public readonly Point3D _end;
    }
